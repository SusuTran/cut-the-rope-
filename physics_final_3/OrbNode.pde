class OrbNode {

  static final float SPRING_LEN = 50;
  static final float SPRING_CONST = 0.005;
  static final float DAMPING = 0.995; //1 is no damping, lower numbers is more aggressive damping
  static final int PART_SIZE = 15;

  PVector position;
  PVector velocity;
  PVector acceleration;
  int osize;
  float mass;
  color c;
  int state;
  PImage node;

  OrbNode candy;

  OrbNode()
  {
    position = null;
  }


  OrbNode(int x, int y) {
    position = new PVector(x, y);
    velocity = new PVector(0, 0);
    acceleration = new PVector(0, 0);
    osize = PART_SIZE;
    state = 0;
    node = loadImage("node.png");
    node.resize(osize*2, osize*2);
  }

  boolean contains(PVector p) {
    float d = position.dist(p);
    return d <= osize;
  }//contains


  void run(OmNom om) {
    acceleration.mult(osize/PART_SIZE);
    velocity.add(acceleration);
    velocity.mult(DAMPING);
    position.add(velocity);
    acceleration.mult(0);

    checkBounds(om);
  }//run

  void applyForce(PVector f) {
    acceleration.add(f);
  }//applyForce

  //boolean checkYBounds() {
  //  boolean check = position.y <= osize/2;
  //  check = check || position.y >= height - osize/2;
  //  return check;
  //}//checkYBounds
  //boolean checkXBounds() {
  //  boolean check = position.x <= osize/2;
  //  check = check || position.x >= width - osize/2;
  //  return check;
  //}//checkXBounds

  void checkBounds(OmNom om)
  {
    boolean checky = position.y <= osize/2;
    checky = checky || position.y >= height - osize/2;
    boolean checkx = position.x <= osize/2;
    checkx = checkx || position.x >= width - osize/2;

    if ( (checkx || checky) && (state == 1))

    {
      println("oh no");
      om.loss();
    }
  }


  void display() {

    if (this.position != null)
    {

      if (candy != null) {
        if (candy.state == 0)
        {
          line(position.x, position.y, candy.position.x, candy.position.y);
        }
      }

      //stroke(0);
      //fill(c);
      //circle(position.x, position.y, osize);
      image(node, position.x - osize, position.y - osize);
    }
  }//candy

  String toString() {
    return position.toString();
  }//toString

  void applySpringForce() {
    if (candy != null) {
      velocity.add(calculateSpringForce(candy));
    }
  }//applySpringForce


  PVector calculateSpringForce(OrbNode other) {
    float dist = position.dist(other.position);
    float displacement = dist - SPRING_LEN;
    float springForce = displacement * SPRING_CONST;

    float xDiff = other.position.x - position.x;
    float yDiff = other.position.y - position.y;
    float xForce = springForce * xDiff/dist;
    float yForce = springForce * yDiff/dist;

    return new PVector(xForce, yForce);
  }//calculateSpringForce
}//OrbNode
